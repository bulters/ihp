...
The instance of the `FrameworkConfig` that is defined in `Config/Config.hs` will be part of the `RequestContext` and is thus available in yocontrollers. To make it more convienient a number of helper functions are available that allow you to directly query your configuration. 
